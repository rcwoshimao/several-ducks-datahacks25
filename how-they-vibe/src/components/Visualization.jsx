  ...
  const ParallelCoordinatesChart = ({ agentData }) => {
    const firstAgentPersonality = agentData.agents && agentData.agents.length > 0
      ? agentData.agents[0].personality
      : {};
  ...
  const AgentAnalytics = ({ data }) => {
    ...
    const sentimentSummary = result?.pie.sentiment_counts || {};
